...
void ModsMenu::render()
{
	...
	if (!mHasAnyMods)
	{
		...
		drawer.printText(global::mSonicFontB, rect, "Nenhum mod instalado!", 1, Color(0.6f, 0.8f, 1.0f, mVisibility));
		rect.y += 22;
		...
		rect.y += 18;
		drawer.printText(global::mOxyfontSmall, rect, "Note que voce tera que reiniciar o Sonic 3.", 1, color);
		rect.y += 14;
	...
	for (size_t tabIndex = minTabIndex; tabIndex <= maxTabIndex; ++tabIndex)
	{
...

		{
			...
			visualRect.x += 24;
			std::string text = (tabIndex == 0) ? "Modos ativos" : "Modos inativos";
			if (tabIndex == mActiveTab)
			{
				text = "* " + text + " *";
				visualRect.x -= 12;
